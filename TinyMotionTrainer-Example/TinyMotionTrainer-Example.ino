...
// Declare BLE service and characteristic
...
void setup() {
  ...
  BLE.setLocalName("Motion Trainer");
...
void loop() {
  ...
  while (isCapturing) {
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      if (numSamplesRead == NUM_SAMPLES) {
        ...
        // Send the detection result via BLE
        gestureCharacteristic.writeValue((byte)maxIndex);
  ...
  // Handle BLE central device connection
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (!isCapturing && IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
        IMU.readAcceleration(aX, aY, aZ);
        IMU.readGyroscope(gX, gY, gZ);

        // Calculate average motion intensity
        float average = fabs(aX / 4.0) + fabs(aY / 4.0) + fabs(aZ / 4.0) + 
                       fabs(gX / 2000.0) + fabs(gY / 2000.0) + fabs(gZ / 2000.0);
        average /= 6.;
      ...
      // If capturing data, continue collecting samples
      if (isCapturing && IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
        // Code for reading and processing IMU data remains the same
        IMU.readAcceleration(aX, aY, aZ);
        IMU.readGyroscope(gX, gY, gZ);
